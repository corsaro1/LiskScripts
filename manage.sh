...
while true; do
	...
	do
		...
		SERVERINFO=$(curl --connect-timeout 6 -s -S "http://"$SERVER""$PRT"/api/loader/status/sync")
		...
		then
			...
			FORGE=$(curl --connect-timeout 6 -s "http://"$SERVER""$PRT"/api/delegates/forging/status?publicKey="$PBK| jq '.enabled')
	...
	then
		...
		do
			...
			then
				...
				ENABLEFORGE=$(curl -s -S --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":"'"$SECRET"'"}' https://"$SERVER""$PRTS"/api/delegates/forging/enable | jq '.success')
				if [ "$ENABLEFORGE" = "true" ];
				then
					...
					CHECKFORGE=$(curl --connect-timeout 6 -s "http://"$SERVER""$PRT"/api/delegates/forging/status?publicKey="$PBK| jq '.enabled')
					if [ "$CHECKFORGE" = "true" ];
					then
	...
	if [ "$FORGINGCOUNT" -gt "1" ]
		then
			...
			do
				...
				curl -s -S --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":"'"$SECRET"'"}' https://"$SERVER""$PRTS"/api/delegates/forging/disable
			done
			...
			do
				...
				if [ "$DIFF" -lt "4" ] && [ "${SERVERSCONSENSUS[$NUM]}" -gt "50" ]; 
				then
					ENABLEFORGE=$(curl -s -S --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":"'"$SECRET"'"}' https://"${SERVERS[$index]}""$PRTS"/api/delegates/forging/enable | jq '.success')
					if [ "$ENABLEFORGE" = "true" ];
					then
	...
	then
		...
		then
			...
			then
				DISABLEFORGE=$(curl -s -S --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":"'"$SECRET"'"}' https://"${SERVERS[$FORGING]}""$PRTS"/api/delegates/forging/disable | jq '.success')
				if [ "$DISABLEFORGE" = "true" ];
				then
					sleep 1 ## Make sure disable had time to take effect
					ENABLEFORGE=$(curl -s -S --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":"'"$SECRET"'"}' https://"${SERVERS[0]}""$PRTS"/api/delegates/forging/enable | jq '.success')
					if [ "$ENABLEFORGE" = "true" ];
					then
						PREVIOUSFORGING=0
						...
						curl -s -S --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":"'"$SECRET"'"}' https://"${SERVERS[1]}""$PRTS"/api/delegates/forging/enable
						PREVIOUSFORGING=1
